# 常见问题

## 启用无线调试太麻烦，有没有途径快速启用？

你可以参考这个视频，将无线调试设置到系统下拉通知栏：
[安卓手机快捷启用无线调试
](https://www.bilibili.com/video/BV1r1UKYjEWj/)

## 这会把安卓设备的屏幕也共享到电脑吗？

不会，__输入流转__ 只会共享键盘和鼠标，不会将安卓设备的屏幕内容投屏到电脑。

## 为什么我在连接设备后无法使用 FX 功能键？

如果你使用的是经过深度修改的安卓系统（如 MIUI / HyperOS），请尝试以下步骤：

1. 打开设备的 开发者选项。
2. 启用“USB 调试”和“USB 调试（安全设置）”。

完成以上设置后，请再次尝试使用快捷键功能。

## 为什么我在启用了“贴边切换”后，在平板端无法通过贴边切回电脑？

可能是因为你在切换到平板时一直按着鼠标按键，请重新按下该按键并重试。

## 我需要自己配置 ADB 吗？

不需要，软件内打包了 adb，会自动调用。

## 为什么我使用有线连接老是连接失败？

这可能是因为你在有线连接下，USB 配置选择了“仅充电”，在这个模式下部分安卓设备无法进行 ADB 的有线连接。
